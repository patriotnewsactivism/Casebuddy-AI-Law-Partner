...
async function callServerProxy(params: CohereParams): Promise<string> {
...
async function callGroqDirect(params: CohereParams): Promise<string> {
...
export const cohereChat = async (params: CohereParams): Promise<string> => {
  const timeout = params.timeoutMs ?? 30000;

  // Try server proxy (has Cohere → Gemini → OpenRouter fallback chain)
  try {
    ...
    console.warn('[cohere] Server proxy failed:', msg.slice(0, 150));
  }
  ...
  if (getGroqKey()) {
    try {
      ...
      console.warn('[cohere] Direct Groq failed:', msg.slice(0, 150));
    }
  }
...
export const parseCohereJson = <T>(text: string, fallback: T): T => {
  try {
