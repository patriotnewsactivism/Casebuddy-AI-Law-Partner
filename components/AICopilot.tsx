...
import { MessageSquare, X, Send, Minimize2, Maximize2, Sparkles, Loader, ThumbsUp, ThumbsDown } from 'lucide-react';
...
const AICopilot: React.FC = () => {
  ...
  const giveFeedback = (msgIndex: number, feedback: 'positive' | 'negative') => {
    setMessages(prev => prev.map((m, i) => i === msgIndex ? { ...m, feedback } : m));
    if (activeCase) {
      recordFeedback('maya', activeCase.id, msgIndex, feedback, { text: messages[msgIndex]?.text ?? '' }).catch(() => {});
    }
  };
                ...
                {m.role === 'assistant' && i > 0 && (
                  <div className="flex items-center gap-1 mt-1 ml-1">
                    <button
                      ...
                      title="Helpful"
                      className={`p-0.5 rounded transition-colors ${m.feedback === 'positive' ? 'text-green-400' : 'text-slate-600 hover:text-green-400'}`}
                    >
                      <ThumbsUp size={11} />
                    </button>
                    <button
                      ...
                      title="Not helpful"
                      className={`p-0.5 rounded transition-colors ${m.feedback === 'negative' ? 'text-red-400' : 'text-slate-600 hover:text-red-400'}`}
                    >
                      <ThumbsDown size={11} />
                    </button>
                  </div>
