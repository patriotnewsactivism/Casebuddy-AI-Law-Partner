exports.handler = async (event) => {
  ...
  const { message, systemInstruction, history = [], caseContext } = body;
  ...
  try {
    ...
    if (!text) return { statusCode: 500, headers: CORS, body: JSON.stringify({ error: 'Empty response from Gemini', raw: data }) };
