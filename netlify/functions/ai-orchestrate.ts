/**
 * Netlify Function — Server-side Firm Command orchestration.
 * Ported from api/ai/orchestrate.ts (Vercel Edge Function).
 *
 * Unlike Vercel's waitUntil(), Netlify keeps the function alive until
 * the handler returns. We run the pipeline inline and return when done.
 * The client watches Supabase Realtime for progress updates.
 *
...
const sbPatch = async (
  supabaseUrl: string, anonKey: string, jwt: string,
...
export default async function handler(req: Request): Promise<Response> {
  ...
  let body: {
    runId: string;
    caseContext: string;
    specialist?: { name: string; title: string; practiceArea: string; systemInstruction: string };
  };
  ...
  // Run the pipeline inline (Netlify keeps the function alive until return)
  ...
  const runAgent = async (taskId: string, prompt: string): Promise<string> => {
    ...
    try {
      ...
      await updateWp(taskId, { status: 'done', content, completed_at: Date.now() });
      return content;
    } catch {
      await updateWp(taskId, { status: 'error', content: ERR_MSG, completed_at: Date.now() }).catch(() => {});
      return ERR_MSG;
    }
  };
  ...
  try {
    ...
    const parallelIds = ['lex-research', 'sol-deadlines', 'doc-draft', 'jules-jury', 'rex-strategy'] as const;
    const parallelPrompts = [
      P.lex(caseContext, maya), P.sol(caseContext, maya), P.doc(caseContext, maya),
      P.jules(caseContext, maya), P.rex(caseContext, maya),
    ];
    const specPrompt = specialist ? P.specialist(caseContext, maya, specialist) : P.lex(caseContext, maya);
    ...
    const titles = [
      'Legal Research Memo', 'Deadlines & SOL', 'First Draft on the Page',
      'Jury & Venue Read', 'Trial Strategy Outline',
    ...
    await patch(`firm_runs?id=eq.${runId}`, { status: 'done', completed_at: new Date().toISOString() });
  } catch {
    try { await patch(`firm_runs?id=eq.${runId}`, { status: 'error' }); } catch { /* best effort */ }
  }

  return json({ ok: true, runId });
}

export const config = { path: "/api/ai/orchestrate" };
