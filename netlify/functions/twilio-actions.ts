/**
 * Netlify Function — Twilio outbound dialer + SMS.
 * Ported from api/twilio-actions.ts
 *
...
async function handleOutboundDialer(req: Request): Promise<Response> {
  ...
  return json({
    ...
    callSid: callData.sid, status: callData.status, to, myCell, caseId: caseId || null,
  });
}
...
export const config = { path: "/api/twilio-actions" };
